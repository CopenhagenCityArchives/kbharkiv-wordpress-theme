@if (is_user_logged_in())
  <li class="profile ml-auto parent" data-level="1" data-color="{{ color(get_field('color_theme', 'option'), 0) }}">
    <a class="d-flex align-items-center sub-menu-btn" href="#">Profil @include("partials.icon", ["icon" => "user", "class" => "ml-auto ml-lg-2"])</a>
...
@else
  <li class="profile ml-auto">
    <a class="d-flex align-items-center" href="{{ get_field('login_page', 'option')['url'] }}">Log ind @include("partials.icon", ["icon" => "lock", "class" => "ml-auto ml-lg-2"])</a>
  </li>
@endif
